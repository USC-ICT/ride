...
{
    /// <summary>
    /// Local Kokoro-backed text-to-speech system that generates audio through a small HTTP service
    ...
    {
        [Header("Endpoint")]
        [SerializeField] private string m_endpoint = "http://127.0.0.1:9003";
        ...
        [Header("Voice")]
        [SerializeField] private string m_fallbackVoice = "af_heart";
        ...
        private IEnumerator StartTextToSpeechGenerationCoroutine(string voice, string text)
        {
            ...
            {
                Debug.LogWarning($"[Kokoro TTS] Synthesis request failed: {webRequest.result} - {webRequest.error}");
            ...
            {
                Debug.LogWarning($"[Kokoro TTS] Failed to parse synthesis response: {exception.Message}");
                CompleteTextToSpeechGeneration(null);
                yield break;
            }
            ...
            {
                Debug.LogWarning($"[Kokoro TTS] Invalid base64 audio payload: {exception.Message}");
                CompleteTextToSpeechGeneration(null);
                yield break;
            }
