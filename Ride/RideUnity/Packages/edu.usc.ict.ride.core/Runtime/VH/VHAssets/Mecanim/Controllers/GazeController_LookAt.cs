...
namespace VHAssets
{
    public class GazeController_LookAt : GazeController
    {
        ...
        public override void InitializeLoadedAsset()
        {
            ...
            // Rebuild bindings to the newly loaded art hierarchy.
            BindGazers();
        }
        ...
        public override void ResetLoadedAsset()
        {
            ...
            // Destroy helper objects and clear any references into the old art hierarchy.
            UnbindGazers();
        }

        private void BindGazers()
        {
            // Rebuild list every time to avoid duplicates and keep AdditionalGazers in sync.
            m_Gazers.Clear();
            m_Gazers.Add(m_Body);
            m_Gazers.Add(m_Head);
            ...
            foreach (GazeObject go in m_Gazers)
            {
                ...
                if (jointGO != null)
                {
                    ...
                    // If we are rebinding, clean up any old helper objects first.
                    if (go.m_AnimationRef != null)
                        Destroy(go.m_AnimationRef);

                    if (go.m_GazeRef != null)
                        Destroy(go.m_GazeRef);
                    ...
                    // Parent to this object so it stays organized, and is destroyed with the character root
                    // if this script lives under it (and also so we can cleanly destroy in ResetLoadedAsset).
                    go.m_AnimationRef.transform.SetParent(transform, false);
                    go.m_GazeRef.transform.SetParent(transform, false);
                }
                else
                {
        ...
        private void UnbindGazers()
        {
            foreach (var go in m_Gazers)
            {
                if (go.m_AnimationRef != null)
                    Destroy(go.m_AnimationRef);
        ...
        void ParentConstraint(Transform Constrained, Transform Constrainer) =>  Constrained.SetPositionAndRotation(Constrainer.position, Constrainer.rotation);
    }
}
