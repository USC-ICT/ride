...
public class MecanimManager : ICharacterController
{
    ...
    #region Properties
    public int NumCharacters { get { return m_characterList.Count; } }
    #endregion
    ...
    public static MecanimManager Get()
    {
        ...
        if (g_instance == null)
        {
            g_instance = FindFirstObjectByType<MecanimManager>();
        }
    ...
    void Awake()
    {
        MecanimCharacter[] mecanimCharacters = FindObjectsByType<MecanimCharacter>(FindObjectsInactive.Include, FindObjectsSortMode.None);
        foreach (MecanimCharacter mecAnimCharacter in mecanimCharacters)
        {
            AddCharacter(mecAnimCharacter);
        }

        m_SpeechFiles = GameObject.FindObjectsByType<AudioSpeechFile>(FindObjectsSortMode.None);
    }

    public override string GetCharacterControllerType()
    {
        return "MecanimManager";
    }

    // locates the audio files in the scene
    ...
    {
        MecanimCharacter ch = m_characterList.Find(delegate (MecanimCharacter c) { return c != null && c.gameObject.activeSelf && c.CharacterName == character; });
        if (ch == null)
        {
    ...
    {
        if (fileName == "unused") fileName = "UnnamedAudioClip";
    ...
    public override void SBTilt(string character, float amount, float repeats, float time)
    {
