...
public class Curve : MonoBehaviour
{
    ...
    public void FollowCurve(GameObject follower, float secondsToReachEnd, Transform lookAtTarget)
    {
        if(lookAtTarget == null)
            StartCoroutine(FollowCurveCoroutine(follower, secondsToReachEnd, default));
        else
            StartCoroutine(FollowCurveCoroutine(follower, secondsToReachEnd, lookAtTarget.position));
    }

    public void FollowCurve(GameObject follower, float secondsToReachEnd, Vector3 lookAtTargetPos)
    {
        StartCoroutine(FollowCurveCoroutine(follower, secondsToReachEnd, lookAtTargetPos));
    }

    IEnumerator FollowCurveCoroutine(GameObject follower, float secondsToReachEnd, Vector3 lookAtTarget)
    {
        float timePassed = 0;
    ...
    public Vector3 GetPosition(float time, bool clamp)
    {
        ...
        try
        {
            ...
            Vector3 interpolation = InterpolateCurve(a.position, b.position, c.position, d.position, u);
